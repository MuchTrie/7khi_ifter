                                    ...
                                    <div className="mb-4">
                                        ...
                                        <div className="flex items-center gap-2 md:gap-4 mt-2 justify-center sm:justify-start">
                                            {[...Array(7)].map((_, i) => (
                                                <div
                                                    key={i}
                                                    className={`w-2 h-2 md:w-3 md:h-3 rounded-full ${i < completedCount ? 'bg-blue-600' : 'bg-gray-300'
                                                        }`}
                                                />
                                            ))}
                                        </div>
                        ...
                        <div className="mb-4 md:mb-6">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
                                {activitiesList.map((activity) => (
                                    ...
                                    >
                                        {/* Badge */}
                                        <div className={`absolute -top-2 -right-2 sm:-top-3 sm:-right-3 w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center border-2 border-white shadow-md z-10 ${activity.completed ? 'bg-green-500' : 'bg-gray-400'
                                            }`}>
                                            <span className="text-white font-bold text-sm sm:text-lg">{activity.id}</span>
                                        </div>

                                        {/* Icon Container */}
                                        <div className={`${activity.color} rounded-t-2xl sm:rounded-t-3xl -mx-3 -mt-3 sm:-mx-6 sm:-mt-6 mb-2 sm:mb-4 p-4 sm:p-8 flex items-center justify-center relative`}>
                                            <div className="text-3xl sm:text-6xl">
                                                {activity.icon}
                                            </div>
                                            {activity.completed && (
                                                <div className="absolute top-1 left-1 sm:top-2 sm:left-2 bg-green-500 text-white text-[10px] sm:text-xs font-bold px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-full">
                                                    ✓
                                                </div>
                                            )}
                                        </div>
